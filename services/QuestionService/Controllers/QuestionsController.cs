...
{    
	...
	public class QuestionsController(QuestionDbContext db) : ControllerBase
    {
        ...
        public async Task<ActionResult<Question>> CreateQuestion(QuestionDto dto)
        {
			...
			if(userId is null || name is null) return BadRequest("Can not get userdetails!");
		...
		public async Task<ActionResult<List<Question>>> GetQuestions(string? tag)
        {
            ...
            {
                query = query.Where(x => x.TagSlug.Contains(tag));
            }
		...
		[Authorize]
		[HttpPut("{id}")]
		public async Task<ActionResult> UpdateQuestion(string id, QuestionDto dto)
        {
            var question = await db.Questions.FindAsync(id);
